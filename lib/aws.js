...
export const uploadFile = async function (file, file_id, sensor_id) {
  const client = new S3({
    ...
    endpoint: process.env.SCW_S3_ENDPOINT || 'https://s3.fr-par.scw.cloud',
    credentials: {
  ...
  const upload = new Upload({
    client,
    params: {
      Bucket: process.env.SCW_S3_BUCKET,
...
export const sendMessageToProcessingQueue = async function (file_id) {
  try {
    const sqsClient = new SQSClient({
      credentials: {
        accessKeyId: process.env.SCW_SQS_ID,
        secretAccessKey: process.env.SCW_SQS_SECRET,
      },
      region: process.env.SCW_REGION || 'fr-par',
      endpoint:
    ...
    const sendMessageCommand = new SendMessageCommand({
      ...
      QueueUrl: process.env.SCW_SQS_QUEUE_URL,
    })
